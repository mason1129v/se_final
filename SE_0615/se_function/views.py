...
def search_rev_view(request):
    y_rev = request.GET.get('y_rev')
    m_rev = request.GET.get('m_rev')

    if y_rev and m_rev:
        y_rev = int(y_rev)
        m_rev = int(m_rev)

        m_rev_rec = RevenueRecord.objects.filter(
            revenue_year=y_rev, revenue_month=m_rev)
        m_target = StoreSalesTarget.objects.filter(
            target_year=y_rev, target_month=m_rev).first()

        if m_rev_rec and m_target:
            tgt_per = [(rcd, rcd.revenue_amount / m_target.target_amount * 100) for rcd in m_rev_rec]
        else:
            tgt_per = []
    else:
        m_rev_rec = RevenueRecord.objects.none()
        m_target = None
        tgt_per = []
...
def store_view(request):
    ...
    status_counts = Counter(status_list)
    total_count = sum(status_counts.values())
    percentages = [(count / total_count) *
                100 for count in status_counts.values()]
    rounded_percentages = [round(percentage) for percentage in percentages]
    counts_list = [count for status, count in status_counts.items()]
    categories_list = list(status_counts.keys())
    record = RevenueRecord.objects.all()
    ...
    if y_rev and m_rev:
        ...
        if m_rev_rec and m_target:
            tgt_per = [(rcd, rcd.revenue_amount / m_target.target_amount * 100)
                    for rcd in m_rev_rec]
        else:
            tgt_per = []
    else:
        m_rev_rec = RevenueRecord.objects.none()
